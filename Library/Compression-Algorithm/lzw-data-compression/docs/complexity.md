...
| --- | --- | --- |
| **Compression Time** | O(N) | Excellent for streaming data. |
...
* **Pros:** Extremely fast compression and decompression (linear time); requires zero pre-computation (no frequency analysis needed); the dictionary does not need to be transmitted alongside the file.
* **Cons:** Performs poorly (and can actually *increase* file size) on highly random or previously compressed data, because it takes time to populate the dictionary with useful sequences; requires careful memory management in memory-constrained embedded systems if dictionary bounding is not implemented.
